...
function getDemoMessages(locale: string): DemoMessages {
  if (isFrenchLocale(locale)) {
    return {
      ...
      heading: 'Nouvelle demande de demo - PcVoyages',
      labels: {
        form: 'Formulaire',
        language: 'Langue',
        name: 'Nom',
  ...
  return {
    ...
    heading: 'New Demo Request - TravelWorks',
    labels: {
      form: 'Form',
      language: 'Language',
      name: 'Name',
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const toEnv = isFrenchLocale(locale)
      ? process.env.RESEND_TO_EMAIL_DEMO_FR ?? 'info@pcvoyages.com, mandreazza@pcvoyages.com'
      : process.env.RESEND_TO_EMAIL_DEMO_EN ?? 'sales@travelworkssolution.com, mandreazza@pcvoyages.com';
    const to = toEnv
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
