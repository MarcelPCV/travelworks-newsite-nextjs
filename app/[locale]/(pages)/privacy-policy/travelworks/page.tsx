...
const policyContentByLocale: Record<string, PolicyContent> = {
  fr: {
    title: 'PcVoyages - Politique de confidentialite',
  ...
  default: {
    title: 'Travelworks - Privacy Policy',
...
export default async function PrivacyPolicyPage({
  ...
  const t = await getTranslations('pages.privacy-policy-travelworks');
  const { locale } = await params;
  setRequestLocale(locale);
  const homeHref = locale === 'en' ? '/' : `/${locale}`;
  const breadcrumbItems: BreadcrumbItem[] = [
    { label: t('breadcrumb.privacy-policy-label'), href: t('breadcrumb.privacy-policy-link') },
    { label: t('breadcrumb.privacy-policy-label2'), href: '#' },
  ];
