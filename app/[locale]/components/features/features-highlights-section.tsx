...
export default function FeaturesHighlightsSection({ cards }: FeaturesHighlightsSectionProps) {
  return (
    <div className="bg-[#e7e7e7] py-5">
      <div
        className="w-full mx-auto max-w-[1600px] rounded-2xl  px-4 py-8 sm:px-6 lg:px-10 lg:py-12"
          ...
          {cards.map((item) => {
            return (
              <div key={item.id} className="grid grid-cols-1 gap-5 sm:grid-cols-[8rem_1fr] sm:gap-6">
                ...
                <div>
                  <h3 className="flex items-start gap-3 text-[1.5rem] font-semibold uppercase leading-tight tracking-[0.01em] text-brand-blue">
                    <HeadingBadge />
