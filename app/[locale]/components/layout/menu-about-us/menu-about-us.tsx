...
import { aboutUsMenuItems } from './menu-about-us-config';
...
export default function MenuAboutUs() {
  ...
  const t = useTranslations('nav.aboutUs');
  ...
  const [visibleCount, setVisibleCount] = useState(aboutUsMenuItems.length);
  ...
  const items = useMemo<ResolvedMenuItem[]>(
    () =>
      aboutUsMenuItems.map((item) => {
        const localizedSlug = aboutUsSlugs[item.canonicalSlug]?.[routeLocale] ?? item.canonicalSlug;
        return {
          key: item.key,
          label: t(item.key),
