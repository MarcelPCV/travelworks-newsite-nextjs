...
import { trainingMenuItems } from './menu-training-config';
...
export default function MenuTraining() {
  ...
  const [visibleCount, setVisibleCount] = useState(trainingMenuItems.length);
  ...
  const items = useMemo<ResolvedMenuItem[]>(
    () =>
      trainingMenuItems.map((item) => {
        const localizedSlug = trainingSlugs[item.canonicalSlug]?.[routeLocale] ?? item.canonicalSlug;
        return {
          key: item.key,
          label: t(item.key),
