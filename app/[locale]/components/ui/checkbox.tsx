...
const Checkbox: React.FC<CheckboxProps> = ({
  ...
  return (
    <div
      className={`flex items-start mb-4 ${error ? 'border border-red-500 p-2 rounded' : ''}`}
      role="group"
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            className={`checkbox-indicator transition duration-300 ${indeterminate ? 'scale-x-100' : ''}`}
          >
            ...
            {/* Indeterminate Line */}
            <div
              className={`absolute h-1 bg-primary transition duration-300 ease-out ${indeterminate ? 'animate-draw' : 'scale-x-0'} rounded-full`}
            ></div>
          </div>
        </div>

        {/* Label Content & Text */}
        <span id={`${id}-label`} className="flex flex-col text-base leading-none">
