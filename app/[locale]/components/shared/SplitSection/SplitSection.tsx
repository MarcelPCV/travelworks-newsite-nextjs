...
export default function SplitSection({
  ...
  const hasValidImage = imageSrc && /^(\/|https?:\/\/|data:)/.test(imageSrc);

  return (
    <section aria-labelledby={headingId} className={clsx('w-full', backgroundClass)}>
      <div
        className={clsx(
      ...
      >
        <div className="grid grid-cols-1 gap-5 overflow-hidden rounded-[1.6rem] md:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)] md:gap-0">
          ...
          >
            {hasValidImage && <Image src={imageSrc} alt={imageAlt} width={800} height={600} />}
          </div>
